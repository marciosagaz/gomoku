...
local Buffer = require "puzzle_buffer"
...
function State:new()
	...
	local windrose = {E=true,W=true,S=true,N=true}
	local step = 1
	self.__index = State
...
function State:expandFrontier()
  local state = self.node.state;
	...
	for _, newPositionOfEmptySpace in ipairs(routes) do
	  newPositionOfEmptySpace = newPositionOfEmptySpace[1]
